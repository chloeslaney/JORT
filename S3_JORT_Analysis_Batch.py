...
os.chdir("C://Users/cs16436/OneDrive - University of Bristol/S3_JORT//")
...
class Experiment:        
    def __init__(self,file_name,error_file):
        ...
        for i in range(0,len(trial_starts)-1): #for each value in range 0 to length (-1 of next trial start) 
            stimulus_start=trial_starts[i]+2 #added change to be exact point 99 appears
            while self.velocity_approach[stimulus_start] > 500: #when changes to less than 500 (i.e. 99 in excel indicates stimulus onset)
                stimulus_start+=1
...
def output(out_files,participant,average_force_ec,max_forces_ec,not_dropped_ec,reaction_time_ec,reaction_time_points,avg_force_points,max_force_points,trials_responded,stimulus_present):
   # out_files.writerow(column_names)
    #out_files.writerow(str(item) for item in column_names)
    for file in out_files:
        file.write(participant)
        file.write(",,,")
    for i in range(len(average_force_ec)): # for each trial in length of average forces 
        ...
        out_files[8].write(str(stimulus_present[i]))
...
def make_out_files(file_name_root):
    ...
    out_files.append(file)
    file=open(file_name_root+"_rt_points.csv","w")
    out_files.append(file)
    file=open(file_name_root+"_avg_force_points.csv","w")
    out_files.append(file)
    file=open(file_name_root+"_max_force_points.csv","w")
    out_files.append(file)
    file=open(file_name_root+"_trials_responded_points.csv","w")
    out_files.append(file)
    file=open(file_name_root+"_trials_stimpresent_.csv","w")
    out_files.append(file)
    return out_files
...
for file_name in participant_file_names:
    ...
    stimulus_present=make_zero(condition_n)
    force_c=0
 #force starts at 0
...
#print (i,average_forces[i],max_forces[i],not_dropped[i],reaction_time[i])
    ...
    avg_force_points=[0.0 for _ in condition_values]
    ...
    max_force_points=[0.0 for _ in condition_values]
    ...
    stimulus_present=[0.0 for _ in condition_values]
    force_c_ec=0
    ...
    for effort_i,effort in enumerate(effort_values): #for each effort value
        for condition_i,condition in enumerate(condition_values): #for each condition value
            force_c_ec=0 
            rt_c_ec=0 
            for trial in experiment.trials: # for each trial
                #if not trial.ignored and trial.condition==condition and abs(trial.effort-effort)<tol: #if same conditions 
                if trial.condition==condition and abs(trial.effort-effort)<tol and trial.stimulus_index<len(trial.times): #add trial to accumulated if same condition and **if stimulus appears (i.e. removes error trial 33 in Block B so only 2 trials included in average)
            ...
            if force_c_ec!=0: #if force not equal to 0
               ...
               # reaction_time_ec[effort_i][condition_i]/=rt_c_ec
                ...
                average_force_ec[effort_i][condition_i]=0 #If force is equal to zero, keep it as zero in average
            if rt_c_ec!=0:
                reaction_time_ec[effort_i][condition_i]/=rt_c_ec #If overall averaged RT is not zero calculate average
            else:
                reaction_time_ec[effort_i][condition_i]=-9999 #If no reaction times, label -9999
                
# For each condition (0, 10, 100, 1000 points), calculate average RT, max force and average force
        
    for condition_i, condition in enumerate(condition_values):
        rt_c_points=0
        force_c_points=0
        for trial in experiment.trials:
            if trial.condition==condition and trial.stimulus_index<len(trial.times):
                start_time_points[condition_i]+=trial.force_start
                end_time_points[condition_i]+=trial.force_end
                avg_force_points[condition_i]+=trial.find_force_average()
                max_force_points[condition_i]+=trial.find_max_force()
                force_c_points+=1
                if trial.reaction_time>0:
                    reaction_time_points[condition_i]+=trial.reaction_time
                    rt_c_points+=1
        trials_responded[condition_i]+=rt_c_points
        stimulus_present[condition_i]+=force_c_points
        if force_c_points!=0:
            start_time_points[condition_i]/=force_c_points
            end_time_points[condition_i]/=force_c_points
            avg_force_points[condition_i]/=force_c_points
            max_force_points[condition_i]/=force_c_points
        else:
            avg_force_points[condition_i]=0
        if rt_c_points!=0:
...
#            print (effort_values[i],condition_values[j],average_force_ec[i][j],max_forces_ec[i][j],not_dropped_ec[i][j],reaction_time_ec[i][j])
    ...
    output(out_files,experiment.participant,average_force_ec,max_forces_ec,not_dropped_ec,reaction_time_ec,reaction_time_points,avg_force_points,max_force_points,trials_responded,stimulus_present)
